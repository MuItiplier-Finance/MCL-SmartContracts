...
makeSuite('bMXXIncentivesController getRewardsBalance tests', (testEnv) => {
  for (const {caseName, emissionPerSecond} of getRewardsBalanceScenarios) {
    it(caseName, async () => {
      ...
      await aDaiMock.handleActionOnBic(userAddress, stakedByUser, totalStaked);
      await advanceBlock((await timeLatest()).plus(100).toNumber());
      ...
      const unclaimedRewards = await bMXXIncentivesController.getRewardsBalance(
        [underlyingAsset],
        userAddress
      );

      const userIndex = await getUserIndex(bMXXIncentivesController, userAddress, underlyingAsset);
      const assetData = (await getAssetsData(bMXXIncentivesController, [{underlyingAsset}]))[0];
