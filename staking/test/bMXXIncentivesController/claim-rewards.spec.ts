...
makeSuite('bMXXIncentivesController claimRewards tests', (testEnv) => {
  for (const {
    caseName,
    ...
    it(caseName, async () => {
      ...
      const {bMXXIncentivesController, stakedbMXX, mToken, aDaiMock} = testEnv;

      const distributionEndTimestamp = await bMXXIncentivesController.DISTRIBUTION_END();
      const userAddress = await bMXXIncentivesController.signer.getAddress();
      ...
      const destinationAddressBalanceBefore = await (toStake ? stakedbMXX : mToken).balanceOf(
        destinationAddress
      );
      await aDaiMock.setUserBalanceAndSupply(stakedByUser, totalStaked);
      const unclaimedRewardsBefore = await bMXXIncentivesController.getUserUnclaimedRewards(
        userAddress
      );
      ...
      const claimRewardsReceipt = await waitForTx(
        await bMXXIncentivesController.claimRewards(
          [underlyingAsset],
          amountToClaim,
          destinationAddress,
          toStake || false
      ...
      const unclaimedRewardsAfter = await bMXXIncentivesController.getUserUnclaimedRewards(
        userAddress
      );

      const destinationAddressBalanceAfter = await (toStake ? stakedbMXX : mToken).balanceOf(
        destinationAddress
      );
