...
makeSuite('bMXXIncentivesController configureAssets', (testEnv: TestEnv) => {
  // custom checks
  it('Tries to submit config updates not from emission manager', async () => {
    const {bMXXIncentivesController, users} = testEnv;
    await expect(
      bMXXIncentivesController.connect(users[2].signer).configureAssets([])
  ...
  for (const {assets, caseName, compareRules, customTimeMovement} of configureAssetScenarios) {
    it(caseName, async () => {
      ...
      const assetsConfigBefore = await getAssetsData(bMXXIncentivesController, assetConfigsUpdate);
      ...
      const txReceipt = await waitForTx(
        await bMXXIncentivesController.configureAssets(assetConfigsUpdate)
      );
      const configsUpdateBlockTimestamp = await getBlockTimestamp(txReceipt.blockNumber);
      const assetsConfigAfter = await getAssetsData(bMXXIncentivesController, assetConfigsUpdate);
